...
public final class Main extends JavaPlugin implements Listener {
  ...
  private void registerPacketModifiers() {
    ProtocolManager pm = ProtocolLibrary.getProtocolManager();
    // remove armor if invisible
    pm.addPacketListener(
      new PacketAdapter(this, PacketType.Play.Server.ENTITY_EQUIPMENT) {
        @Override
        public void onPacketSending(PacketEvent e) {
          PacketContainer packet = e.getPacket();
          if (packet.getIntegers().read(1) < 1) return; // non-armor equipment
          int eid = packet.getIntegers().read(0);
          Player receiver = e.getPlayer();
          for (Player sender : Bukkit.getOnlinePlayers()) {
            if (sender.getEntityId() == eid) {
              for (PotionEffect potion : sender.getActivePotionEffects()) {
                if (true || potion.getType() == PotionEffectType.INVISIBILITY) {
                  PacketContainer newpacket = packet.deepClone();
                  newpacket.getItemModifier().write(0, new ItemStack(Material.AIR));
  ...
  public void onPlayerItemConsume(PlayerItemConsumeEvent e) {
    if (e.getItem().getType() == Material.POTION &&
        ((PotionMeta) e.getItem().getItemMeta()).hasCustomEffect(PotionEffectType.INVISIBILITY)) {
      for (int i=1;i<=4;i++) {
    ...
    if (e.getEntity() instanceof Player) {
      if (e.getCause() != EntityDamageEvent.DamageCause.FALL) {
        ...
        if (p.hasPotionEffect(PotionEffectType.INVISIBILITY)) {
          ...
          p.sendMessage(ChatColor.RED + "You took damage so you lost your invisibility!");
        }
      }
    }
  }
